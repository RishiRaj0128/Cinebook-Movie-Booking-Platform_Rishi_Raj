...
const API_BASE =
  ...
  'https://cinebook-api-6cw7.onrender.com/api';
...
async function handleResponse<T>(res: Response): Promise<T> {
  ...
  if (text && text.trim().length > 0) {
    try {
      ...
      throw new Error(
        `Server response error (${res.status}): ${text.substring(0, 150)}`
      );
    }
  }
...
export const api = {
  ...
  async getMovies(genre?: string, language?: string): Promise<Movie[]> {
    ...
    try {
      ...
      if (data && data.length > 0) {
        try {
          localStorage.setItem('cinebook_cached_movies', JSON.stringify(data));
        } catch {}
        return data;
      }
    } catch {
  ...
  async getShows(movieId: string, date: string): Promise<Show[]> {
    const res = await fetch(
      `${API_BASE}/shows?movieId=${movieId}&date=${date}`
    );
  ...
  saveLocalBooking(booking: Booking): void {
    try {
      ...
      localStorage.setItem(
        'cinebook_local_bookings',
        JSON.stringify([booking, ...filtered])
      );
    } catch (e) {
  ...
  async getPublicBooking(bookingId: string): Promise<Booking> {
    ...
    const res = await fetch(
      `${API_BASE}/bookings/public/${encodeURIComponent(bookingId)}`
    );
    return handleResponse<Booking>(res);
  },
  ...
  async createPaymentOrder(bookingId: string): Promise<PaymentOrderResponse> {
    const res = await fetch(
      `${API_BASE}/payments/create-order?bookingId=${bookingId}`,
      {
        method: 'POST',
        headers: getHeaders(),
      }
  ...
  async getAiRecommendations(query: string): Promise<string> {
    const res = await fetch(
      `${API_BASE}/ai/recommend?query=${encodeURIComponent(query)}`,
      {
        method: 'POST',
        headers: getHeaders(),
      }
